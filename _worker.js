...
async function 生成VLESS配置(userID, hostName, port, path) {
    const remark = `VLESS节点 - ${hostName}:${port}`;
    const vlessConfig = buildXrayVLOutbound(
        remark,
        hostName,
        port,
        hostName,
...
async function 生成随机节点订阅(userID, hostName, ports, paths) {
    const 配置列表 = [];
    for (const port of ports) {
        for (const path of paths) {
            const vlessConfig = await 生成VLESS配置(userID, hostName, port, path);
