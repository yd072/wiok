...
async function 生成配置信息(uuid, hostName, sub, UA, RproxyIP, _url, fakeUserID, fakeHostName, env) {
    ...
    // 如果配置了 subConverter，则优先使用它
    if (subConverter && subConverter.trim() !== '') {
        console.log("检测到 subConverter 配置，将使用外部转换流程。");

        let sourceUrl;
        // 如果用户仍然在 URL 中指定了 sub，则优先使用用户指定的 sub
        if (sub) {
            console.log(`使用用户指定的 sub 源: ${sub}`);
            const match = sub.match(/^(?:https?:\/\/)?([^\/]+)/);
            sub = match ? match[1] : sub;
            const subs = 整理(sub);
            sub = subs.length > 1 ? subs[0] : sub;
            sourceUrl = `${subProtocol}://${sub}/sub?host=${fakeHostName}&uuid=${fakeUserID + atob('JmVkZ2V0dW5uZWw9Y21saXUmcHJveHlpcD0=') + RproxyIP}&path=${encodeURIComponent('/')}`;
        } else {
            // 否则，使用 Worker 自身作为节点源
            console.log("未指定 sub 源，将使用 Worker 自身作为节点源提供给 subConverter。");
            sourceUrl = `https://${hostName}/${fakeUserID + _url.search}`;
            if (noTLS === 'true') {
                if (_url.search) sourceUrl += '&notls';
                else sourceUrl += '?notls';
            }
        }
        console.log(`提供给 subConverter 的源 URL: ${sourceUrl}`);

        let targetUrl;
        let isBase64 = true;

        if (isClashRequest) {
            targetUrl = `${subProtocol}://${subConverter}/sub?target=clash&url=${encodeURIComponent(sourceUrl)}&insert=false&config=${encodeURIComponent(subConfig)}&emoji=${subEmoji}&list=false&tfo=false&scv=true&fdn=false&sort=false&new_name=true`;
            isBase64 = false;
        } else if (isSingboxRequest) {
            targetUrl = `${subProtocol}://${subConverter}/sub?target=singbox&url=${encodeURIComponent(sourceUrl)}&insert=false&config=${encodeURIComponent(subConfig)}&emoji=${subEmoji}&list=false&tfo=false&scv=true&fdn=false&sort=false&new_name=true`;
            isBase64 = false;
        } else if (isLoonRequest) {
            targetUrl = `${subProtocol}://${subConverter}/sub?target=loon&url=${encodeURIComponent(sourceUrl)}&insert=false&config=${encodeURIComponent(subConfig)}&emoji=${subEmoji}&list=false&tfo=false&scv=true&fdn=false&sort=false&new_name=true`;
            isBase64 = false;
        } else {
            targetUrl = sourceUrl; 
        }
        ...
        try {
            const response = await fetch(targetUrl, { headers: { 'User-Agent': UA + atob('IENGLVdvcmtlcnMtZWRnZXR1bm5lbC9jbWxpdQ==') } });
            if (!response.ok) {
                 throw new Error(`从 subConverter 获取内容失败: ${response.status} ${response.statusText}`);
            }
            ...
            if (isBase64 && targetUrl === sourceUrl) {
                 const rawContentResponse = await fetch(sourceUrl, { headers: { 'User-Agent': 'CF-Workers-SUB' } });
                 const rawContent = await rawContentResponse.text();
                 return 恢复伪装信息(btoa(rawContent), uuid, hostName, fakeUserID, fakeHostName, true);
            }
            return 恢复伪装信息(content, uuid, hostName, fakeUserID, fakeHostName, isBase64);
        } catch (error) {
            console.error('Error fetching external subconverter:', error);
            return `从 subconverter 获取内容时出错: ${error.message}`;
        }

    } else {
        // 如果未配置 subConverter，则回退到内置生成逻辑
        console.log("未配置 subConverter，回退到内置订阅生成逻辑。");
        
        // 浏览器访问时，显示配置页面
        if (userAgent.includes('mozilla') && !subParams.some(_searchParams => _url.searchParams.has(_searchParams))) {
            return generateHtmlPage(uuid, hostName, sub, UA, RproxyIP, _url, fakeUserID, env, fakeHostName);
        }
        ...
        if (_url.pathname === `/${fakeUserID}`) {
            const base64Content = 生成本地订阅(nodeObjects);
            return atob(base64Content);
        }

        if (isClashRequest) {
            const clashConfig = generateClashConfig(nodeObjects);
            return 恢复伪装信息(clashConfig, uuid, hostName, fakeUserID, fakeHostName, false);
        }
        
        if (isSingboxRequest) {
            const singboxConfig = generateSingboxConfig(nodeObjects);
            return 恢复伪装信息(singboxConfig, uuid, hostName, fakeUserID, fakeHostName, false);
        }

        if (isLoonRequest) {
            const loonConfig = generateLoonConfig(nodeObjects);
            return 恢复伪装信息(loonConfig, uuid, hostName, fakeUserID, fakeHostName, false);
        }
        
        // 默认情况或请求 base64，则返回 vless 链接
        const base64Content = 生成本地订阅(nodeObjects);
        return 恢复伪装信息(base64Content, uuid, hostName, fakeUserID, fakeHostName, true);
    }
}

// 辅助函数：生成 HTML 页面 (从原代码中提取)
function generateHtmlPage(uuid, hostName, sub, UA, RproxyIP, _url, fakeUserID, env, fakeHostName) {
    ...
    if (subConverter && subConverter.trim() !== '') {
        订阅器 += `您的订阅由外部 subConverter 处理: ${subProtocol}://${subConverter}${判断是否绑定KV空间}<br>`;
    } else {
        订阅器 += `您的订阅内容由 内置生成器 提供${判断是否绑定KV空间}<br>`;
