...
export default {
	async fetch(request, env, ctx) {
		try {
			...
			// 1. 添加测速端点
			// 这个端点专门用于客户端进行延迟测试。它返回一个最小的响应，并带有允许跨域的头。
			if (url.pathname === '/speedtest') {
				return new Response(null, {
					status: 204, // No Content，最轻量的成功响应
					headers: {
			...
			if (!upgradeHeader || upgradeHeader !== 'websocket') {
				...
				// 【方案一：核心安全修复】在这里修改了判断逻辑
					...
					if (userAgent && userAgent.includes('mozilla')) {
						return new Response(secureProtoConfig, { // 直接返回HTML内容
							status: 200,
							headers: {
								"Content-Type": "text/html;charset=utf-8",
		...
		const 节点配置页 = `
			...
			<head>
				...
				<style>
					...
					.copy-button:hover, .btn:hover:not(:disabled) {
			...
			<body>
				<div class="container">
					...
					<div class="section">
						<div class="section-title">⚡ 真实延迟优选IP</div>
						<p>点击下方按钮开始测试，脚本将从CF优选IP段中随机选择IP进行延迟测试，并按速度排序。</p>
						<div class="button-group">
							<button class="btn" id="start-test-btn">开始测速</button>
							<button class="btn btn-secondary" id="copy-results-btn" style="display: none;">复制优选IP</button>
						</div>
						<div id="test-status" style="margin-top: 10px;"></div>
						<table class="test-results" id="results-table" style="display: none;">
							<thead>
								<tr>
									<th>IP 地址</th>
									<th>延迟 (ms)</th>
				...
				<script>
					...
					const cfCIDRs = ['104.16.0.0/14', '104.21.0.0/16', '188.114.96.0/20'];
					const testCount = 15; // 测试的IP数量
					const testPort = '443'; // 默认测试的端口
					...
					function generateRandomIPFromCIDR(cidr) {
						...
						const maskBits = parseInt(mask, 10);
						const hostBits = 32 - maskBits;
						const maxHosts = Math.pow(2, hostBits) - (hostBits === 32 ? 0 : 2); // Exclude network and broadcast
						const randomOffset = Math.floor(Math.random() * maxHosts) + 1;
						return intToIp(baseInt + randomOffset);
					}

					async function ping(ip) {
						const url = \`https://\${ip}:${testPort}/speedtest\`;
						const controller = new AbortController();
						const signal = controller.signal;
						...
						try {
							// 使用 no-cors 模式，因为我们不关心响应内容，只关心连接成功与否和耗时
							await fetch(url, { method: 'GET', signal, mode: 'no-cors' });
							const latency = Date.now() - startTime;
							clearTimeout(timeoutId);
							return { ip, latency, status: 'success' };
						} catch (e) {
							clearTimeout(timeoutId);
							// 如果是超时错误，延迟时间无意义
							if (e.name === 'AbortError') {
								return { ip, latency: 'Timeout', status: 'failed' };
							}
							return { ip, latency: 'Error', status: 'failed' };
						}
					}

					function updateTable(ip, latency = '', status = 'testing') {
						let row = document.getElementById(\`row-\${ip.replace(/\\./g, '-')}\`);
						if (!row) {
							row = document.createElement('tr');
							row.id = \`row-\${ip.replace(/\\./g, '-')}\`;
							row.innerHTML = \`<td>\${ip}</td><td class="latency"></td><td class="status"></td>\`;
						...
						statusCell.textContent = status === 'testing' ? '测试中...' : (status === 'success' ? '成功' : '失败');
					...
					function sortAndReorderTable() {
						...
						rows.sort((a, b) => {
							const aLatency = a.querySelector('.latency').textContent;
							const bLatency = b.querySelector('.latency').textContent;
							const aIsNumber = !isNaN(parseFloat(aLatency));
							const bIsNumber = !isNaN(parseFloat(bLatency));

							if (aIsNumber && bIsNumber) return parseFloat(aLatency) - parseFloat(bLatency);
					...
					startTestBtn.addEventListener('click', async () => {
						...
						while(ipsToTest.size < testCount) {
							const randomCIDR = cfCIDRs[Math.floor(Math.random() * cfCIDRs.length)];
							ipsToTest.add(generateRandomIPFromCIDR(randomCIDR));
						}
						
						const ipArray = Array.from(ipsToTest);
						let testedCount = 0;
						testStatus.textContent = \`正在测试 \${testedCount}/\${ipArray.length}...\`;

						const promises = ipArray.map(ip => {
							updateTable(ip); // Pre-populate table
							return ping(ip).then(result => {
								testedCount++;
								testStatus.textContent = \`正在测试 \${testedCount}/\${ipArray.length}...\`;
								updateTable(result.ip, result.latency, result.status);
						...
						// 按延迟排序
						sortedResults.sort((a,b) => a.latency - b.latency);

						testStatus.textContent = \`测试完成！共 \${sortedResults.length} 个IP可用。\`;
						sortAndReorderTable();
					...
					copyResultsBtn.addEventListener('click', () => {
						const ipsToCopy = sortedResults.map(r => \`\${r.ip}:${testPort}#CF优选IP_\${r.latency}ms\`).join('\\n');
						navigator.clipboard.writeText(ipsToCopy).then(() => {
							alert(\`\${sortedResults.length} 个优选IP已复制到剪贴板！\`);
						}).catch(err => {
							console.error('复制失败:', err);
							alert('复制失败，请检查浏览器权限。');
						});
